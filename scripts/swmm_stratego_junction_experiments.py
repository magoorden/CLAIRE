...
def swmm_control(swmm_inputfile, orifice_id, basin_id, time_step, csv_file_basename, controller,
    ...
    with Simulation(swmm_inputfile) as sim:
        ...
        rain.append(0)
        junction_inflow.append(j2.total_inflow)
        junction_outflow.append(j2.total_outflow)
        junction_depth.append(j2.depth)
    ...
    with open(output_csv_file, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["time", "water_depth_basin1", "water_depth_basin2",
                         "rain", "junction_inflow", "junction_outflow", "junction_depth"])
        for i, j, k, l, m, n, o in zip(time_series, water_depth_basin1, water_depth_basin2,
                                       rain, junction_inflow, junction_outflow, junction_depth):
            i = i.strftime('%Y-%m-%d %H:%M')
            writer.writerow([i, j, k, l, m, n, o])
...
def main():
    ...
    swmm_inputfile = os.path.join(base_folder, swmm_folder, "swmm_junction_experiments.inp")
    ...
    swmm_results = "swmm_junction_experiments"

...
def swmm_control(swmm_inputfile, orifice_id, basin_id, time_step, csv_file_basename, controller,
    ...
    with Simulation(swmm_inputfile) as sim:
        ...
        su2 = Nodes(sim)['SU2']
        stream1 = Nodes(sim)['J2']
        stream2 = Nodes(sim)['J3']
        ...
        water_depth_stream1.append(stream1.depth)
        water_depth_stream2.append(stream2.depth)
        rain.append(0)
    ...
    with open(output_csv_file, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["time", "water_depth_basin1", "water_depth_basin2", "water_depth_steam1",
                         "water_depth_stream2", "orifice_setting", "rain", "forecast_low",
                         "forecast_high", "forecast_int"])
        for i, j, k, l, m, n, o, p, q, r in zip(time_series, water_depth_basin1, water_depth_basin2,
                                                water_depth_stream1, water_depth_stream2,
...
def main():
    ...
    swmm_inputfile = os.path.join(base_folder, swmm_folder, "swmm_demo3_temp.inp")
    ...
    orifice_id = "OR1"
    basin_id = "SU1"
    time_step = 60 * 5  # 60 seconds/min x 5 -> 5 min
    swmm_results = "swmm_demo3_temp_results"
